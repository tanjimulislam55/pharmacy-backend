...
class StockDAL(BaseDAL[Stock, StockCreate, StockUpdate]):
    def increase_stock_quantity_filtered_by_medicine_id_without_commit(
        self, db: Session, medicine_id: int, quantity: int, pharmacy_id: int
    ) -> Stock:
        db.query(self.model).filter(
            self.model.medicine_id == medicine_id, self.model.pharmacy_id == pharmacy_id
        ).update(
    ...
    def read_one_filtered_by_medicine_id(
        self, db: Session, medicine_id: int, pharmacy_id: int
    ) -> Optional[Stock]:
        return (
            db.query(self.model)
            .filter(
    ...
    def update_one_filtered_by_medicine_id(
        self, db: Session, medicine_id: int, obj_in: StockUpdate, pharmacy_id: int
    ) -> Stock:
        db.query(self.model).filter(
            self.model.medicine_id == medicine_id, self.model.pharmacy_id == pharmacy_id
    ...
    def update_one_filtered_by_medicine_id_with_commit(
        ...
        db.commit()
        return self.read_one_filtered_by_medicine_id(db, medicine_id, pharmacy_id)
